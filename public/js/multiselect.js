...
;(function (factory) {
        ...
        **/
        ...
        Multiselect.prototype = {
            ...
            events: function() {
                ...
                if (self.options.search && self.options.search.$left) {
                    //self.options.search.$left.on('change', function(e) {
					$('#searchusersbyccleft').on('click', function(e) {
						var value = $('#searchusersbyccleftq').val().toUpperCase();
                        if (self.callbacks.fireSearch(value)) {
                            var $toShow = self.$left.find('option:search("' + value + '")').mShow();
                            var $toHide = self.$left.find('option:not(:search("' + value + '"))').mHide();
					...
					/*$('#searchrespondersbyccleft').on('click', function(e) {
						var value = $('#searchrespondersbyccleftq').val().toUpperCase();
                        if (self.callbacks.fireSearch(value)) {
                            var $toShow = self.$left.find('option:search("' + value + '")').mShow();
                            var $toHide = self.$left.find('option:not(:search("' + value + '"))').mHide();
                            var $grpHide = self.$left.find('option').closest('optgroup').mHide();
                ...
                if (self.options.search && self.options.search.$right) {
                    //self.options.search.$right.on('change', function(e) {
					$('#searchusersbyccright').on('click', function(e) {
						var value = $('#searchusersbyccrightq').val().toUpperCase();
                        if (self.callbacks.fireSearch(value)) {
                            var $toShow = self.$right.find('option:search("' + value + '")').mShow();
                            var $toHide = self.$right.find('option:not(:search("' + value + '"))').mHide();
    ...
    $.fn.mShow = function() {
		this.removeClass('hidden').show(0);
    ...
    $.expr[":"].search = function(elem, index, meta) {
		//return $(elem).text().startsWith(meta[3]);
